...
trait WhatsAppTrait
{
    ...
    public function sendWhatsAppMessage($message, $message_type)
    {
        ...
        try { 
            ...
            //     $header = mb_convert_encoding($header, 'UTF-8', 'auto'); // Ensure UTF-8 encoding
            //     if (strlen($header) > 60) {
            //         $header = substr($header, 0, 57) . '...';
            //     }
            //     $footer = $message->footer_text ?? '';
            //     $footer = mb_convert_encoding($footer, 'UTF-8', 'auto'); // Ensure UTF-8 encoding
            //     if (strlen($footer) > 60) {
            ...
            //         $title = $this->ensureUtf8($button['text']);
            //         if (strlen($title) >= 1 && strlen($title) <= 20) {
            //             $buttons[] = new Button($button['id'], $title);
            //         } else {
            //             if (strlen($title) > 20) {
            ...
            //     $action = new ButtonAction($buttons);
            //     // Process header
            //     $header = isset($message->header_text) ? $this->ensureUtf8($message->header_text) : '';
            //     if (strlen($header) > 60) {
            //         $header = substr($header, 0, 57) . '...';
            //     }
            //     // Process footer
            //     $footer = isset($message->footer_text) ? $this->ensureUtf8($message->footer_text) : '';
            //     if (strlen($footer) > 60) {
            //         $footer = substr($footer, 0, 57) . '...';
            //     }
            //     $response = $whatsapp_cloud_api->sendButton(
            ...
            Log::info('whatsapp interactive message body check', [$message_body]);
